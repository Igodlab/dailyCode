...
sumFunction [] _ = Left []
sumFunction xs n = case filtered of
                    [] -> Left []
                    [(x,y):_]  -> Right [x, y]
  where
    combinationExpand :: [a] -> [a] -> [a]
    combinationExpand _ [] = []
    combinationExpand acc (x:xs) = combinationExpand (acc ++ [(x, xi) | xi <- xs]) xs

    filtered = filter (\(x, y) -> x + y == n) $ combinationExpand [] xs
